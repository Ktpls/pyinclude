...
public:
    ...
    pipe(pipe &&c_) : c(std::move(c_.c)) {}
    static auto of(const std::vector<eT> &c_)
    {
        return pipe(c_);
    }
    static auto of(const std::initializer_list<eT> &c_)
    {
        return pipe<eT>(std::vector<eT>(c_));
    }
    ...
    auto map(std::function<rT(const eT &)> mapper)
    {
        ...
        ret.reserve(c.size());
        this->foreach ([&ret, &mapper](auto i)
                       { ret.push_back(mapper(i)); });
        return pipe<rT>(ret);
    }
    auto enumerate()
    {
